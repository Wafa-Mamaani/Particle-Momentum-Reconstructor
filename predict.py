...
def run_inference(data_dir: str, weights_path: str, output_dir: str):
    '''
    Loads the trained model and predicts pT for the held-out test set.
    The scaled predictions and targets are converted back to MeV/c.
    '''
    ...
    if X_test.shape[0] != y_test_scaled.shape[0]:
        raise ValueError(
            'Test features and targets must contain the same number of rows.'
        )
...
def main():  # pragma: no cover
    ...
    parser.add_argument(
        ...
        help='Directory containing X_test.npy.',
    )
    parser.add_argument(
        '--weights',
        type=str,
        default='weights/best_model.pth',
        help='Path to the .pth weights.',
    )
    parser.add_argument(
        '--outdir',
        type=str,
